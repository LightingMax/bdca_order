...
def parse_trip_info_manual(text):
    """
    手动解析版：基于实际观察到的文本结构
    从调试信息可以看到：
    15: '1'
    ...
    try:
        ...
        while i < len(lines):
            ...
            if line.isdigit():
                ...
                # 按照实际观察到的结构解析
                # 序号 -> 服务商 -> 车型 -> 上车时间 -> 城市 -> 起点(可能2行) -> 终点 -> 金额
...
def test_trip_extraction():
    ...
    try:
        ...
        # 测试改进版解析
        print(f"\n🔄 测试改进版解析...")
        trips = parse_trip_info_improved(full_text)
        
        if trips:
            print(f"✅ 改进版解析成功，解析到 {len(trips)} 个行程信息:")
