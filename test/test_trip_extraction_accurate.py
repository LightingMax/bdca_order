...
def parse_trip_info_accurate(text):
    """
    准确版：基于实际PDF的垂直字段结构解析
    每个行程的字段是垂直排列的：
    1
    旅程易到
    旅程易到经济型
...
def test_trip_extraction():
    ...
    try:
        ...
        # 测试准确版解析
        print(f"\n🔄 测试准确版解析...")
        trips = parse_trip_info_accurate(full_text)
        
        if trips:
            print(f"✅ 准确版解析成功，解析到 {len(trips)} 个行程信息:")
