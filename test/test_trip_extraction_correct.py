...
def parse_trip_info_fixed(text):
    """
    修复版：基于实际观察到的文本结构进行准确解析
    关键是要正确识别起点和终点的分界
    """
    trips = []
...
def test_trip_extraction():
    ...
    try:
        ...
        # 测试正确版解析
        print(f"\n🔄 测试正确版解析...")
        trips = parse_trip_info_correct(full_text)
        
        if trips:
            print(f"✅ 正确版解析成功，解析到 {len(trips)} 个行程信息:")
